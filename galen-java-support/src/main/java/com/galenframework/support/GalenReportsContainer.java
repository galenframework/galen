...
public enum GalenReportsContainer {
     ...
     */
    public static synchronized GalenReportsContainer get() {
        return INSTANCE;
    }

    public synchronized TestReport registerTest(Method method) {
        GalenTestInfo testInfo = GalenTestInfo.fromMethod(method);
        tests.add(testInfo);
        return testInfo.getReport();
    }

    public synchronized TestReport registerTest(String name, List<String> groups) {
        GalenTestInfo testInfo = GalenTestInfo.fromString(name, groups);
        tests.add(testInfo);
        return testInfo.getReport();
    }

    public synchronized TestReport registerTest(GalenTestInfo testInfo) {
        tests.add(testInfo);
        return testInfo.getReport();
    }

    public synchronized List<GalenTestInfo> getAllTests() {
        return tests;
    }
}
