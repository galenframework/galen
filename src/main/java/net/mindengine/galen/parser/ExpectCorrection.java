...
public class ExpectCorrection implements Expectation<CorrectionsRect> {
    ...
    @Override
    public CorrectionsRect read(final StringCharReader reader) {
        boolean started = false;
        final StringBuilder numbersText = new StringBuilder();
    ...
    private CorrectionsRect processCorrection(final String numbersText) {
        if (!numbersText.isEmpty()) {
            final String values[] = numbersText.split(",");
            if (values.length == 4) {
                ...
                throw new SyntaxException(UNKNOWN_LINE, "Wrong number of arguments in corrections: " + values.length);
            }
        } else {
            throw new SyntaxException(UNKNOWN_LINE, "Error parsing corrections. No values provided");
        }
    }

    private Correction readCorrection(final String value) {
        if (value.length() == 0) {
            throw new SyntaxException("Incorrect correction. Don't use empty values");
        }

        final char symbol = value.charAt(0);
