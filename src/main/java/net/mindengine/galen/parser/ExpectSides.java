...
public class ExpectSides implements Expectation<List<Side>> {
    ...
    @Override
    public List<Side> read(final StringCharReader reader) {
        final ExpectWord expectWord = new ExpectWord().stopOnTheseSymbols(',');

        final List<Side> sides = new LinkedList<Side>();

        while (reader.hasMore()) {
            final String side = expectWord.read(reader);
            if (!side.isEmpty()) {
